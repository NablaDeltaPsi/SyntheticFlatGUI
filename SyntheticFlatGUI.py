...
# astropy (5.0.3), numpy (1.26.4), opencv-python (4.10.0.84), rawpy (0.23.1), scipy (1.13.1)
...
import glob, os, sys, copy, datetime as dt
...
def calc_slope(image_, color_index=0, ignore_region=10, sigma_clip=2):
    ...
    for row in range(height):
        rowmeans.append(sigma_clip_mean(image[row, :], sigma_clip=sigma_clip))
    slope_y = np.mean(np.diff(rowmeans)) * image_.shape[0]

    colmeans = []
    for col in range(width):
        colmeans.append(sigma_clip_mean(image[:, col], sigma_clip=sigma_clip))
    slope_x = np.mean(np.diff(colmeans)) * image_.shape[1]
    return slope_x, slope_y

def corr_gradient(image, sigma_clip=3):
    ...
    for c in range(colors):
        ...
        slope_x, slope_y = calc_slope(image, color_index=c, sigma_clip=sigma_clip, ignore_region=IGNORE_EDGE)
...
class Image():
    ...
    def gradcorr(self):
        if self.debug:
            return
        self.image = corr_gradient(self.image)
...
class NewGUI():
    ...
    def ask_bias_file(self):
        ...
        user_input_file = askopenfilename(initialdir=self.lastpath, multiple=True,
              filetypes=[('RAW format (supported)', RAWTYPES), ('Image format (not supported)', FILETYPES), ('all', '.*')])
